...
const initMessagingTables = async () => {
  ...
  // Pickle Courts table (copied from services)
  await pool.query(`
    CREATE TABLE IF NOT EXISTS pickle_courts (
        id SERIAL PRIMARY KEY,
        owner_email TEXT,
        name VARCHAR(255) NOT NULL,
        duration INTEGER DEFAULT 30,
        base_price DECIMAL(10, 2) DEFAULT 0,
        hourly_prices JSONB DEFAULT '[]'::jsonb,
        description TEXT,
  ...
  // Migration for pickle_courts (drop price, add base_price and hourly_prices if missing)
  try {
    await pool.query(`ALTER TABLE pickle_courts ADD COLUMN IF NOT EXISTS base_price DECIMAL(10, 2) DEFAULT 0`);
    await pool.query(`ALTER TABLE pickle_courts ADD COLUMN IF NOT EXISTS hourly_prices JSONB DEFAULT '[]'::jsonb`);
    await pool.query(`ALTER TABLE pickle_courts ADD COLUMN IF NOT EXISTS latitude DECIMAL(10, 8)`);
    await pool.query(`ALTER TABLE pickle_courts ADD COLUMN IF NOT EXISTS longitude DECIMAL(11, 8)`);
    await pool.query(`ALTER TABLE pickle_courts DROP COLUMN IF EXISTS price`);
  } catch(e) {
    console.error('Error migrating pickle_courts:', e);
  }
};
  ...
  try {
    ...
    if (paymentMethod === 'corporate' && corporateAccountNumber) {
      const corpResult = await pool.query(
        "SELECT id, balance, credit_limit, status FROM pickle_corporate_accounts WHERE UPPER(account_number) = UPPER($1)",
      ...
      // Update balance
      await pool.query('UPDATE pickle_corporate_accounts SET balance = balance + $1 WHERE id = $2', [amount, corporateAccountId]);
    }
    ...
    // Automatically create a notification for the Court Owner
    try {
      const courtOwnerResult = await pool.query(
        `SELECT owner_email FROM pickle_courts WHERE LOWER(name) = LOWER($1) OR $1 ILIKE '%' || name || '%' OR name ILIKE '%' || $1 || '%' LIMIT 1`,
        [serviceType]
      );
      ...
      await pool.query(
        ...
        [
          ...
          email,
          '🎾 New Court Booking!',
          `${fullName} booked ${serviceType} on ${preferredDate} at ${preferredTime}.`
        ]
      );
    } catch (notifErr) {
      console.error('Owner notification creation error:', notifErr);
    }
...
app.get('/api/available-slots', async (req, res) => {
  try {
    ...
    // Get booked slots for the date (using ::date casting for PostgreSQL timestamp compatibility)
    let queryParams = [date];
    let queryStr = `SELECT preferred_time, status, service_type FROM pickle_appointment WHERE preferred_date::date = $1::date AND status != 'cancelled'`;
    
    if (serviceType && serviceType.trim().length > 0) {
      const cleanServiceType = serviceType.trim();
      queryStr += ` AND (
        LOWER(service_type) = LOWER($2) 
        OR $2 ILIKE '%' || service_type || '%' 
        OR service_type ILIKE '%' || $2 || '%'
        OR LOWER(REGEXP_REPLACE(service_type, '[^a-zA-Z0-9]', '', 'g')) = LOWER(REGEXP_REPLACE($2, '[^a-zA-Z0-9]', '', 'g'))
      )`;
      queryParams.push(cleanServiceType);
    }
    ...
    const blockedSlots = bookedResult.rows
      .filter(row => row.status === 'blocked')
      .map(row => normalizeTime(row.preferred_time));
  ...
  try {
    const result = await pool.query(
      `SELECT a.*, 
              ...
              ca.account_number AS corporate_account_number FROM pickle_appointment a
       LEFT JOIN pickle_corporate_accounts ca ON a.corporate_account_id = ca.id
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT a.*, 
             ...
             r.plate_number,
             r.phone as rider_phone FROM pickle_appointment a
...
app.patch('/api/appointments/:id', async (req, res) => {
  try {
    ...
    let query = 'UPDATE pickle_appointment SET status = $1, updated_at = CURRENT_TIMESTAMP';
...
app.get('/api/corporate-accounts', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM pickle_corporate_accounts ORDER BY company_name ASC');
...
app.post('/api/corporate-accounts', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO pickle_corporate_accounts (account_number, company_name, contact_person, contact_email, contact_phone, credit_limit)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
...
app.put('/api/corporate-accounts/:id', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE pickle_corporate_accounts SET company_name = COALESCE($1, company_name),
...
app.delete('/api/corporate-accounts/:id', async (req, res) => {
  try {
    ...
    await pool.query(' FROM pickle_appointment SET corporate_account_id = NULL WHERE corporate_account_id = $1', [req.params.id]).catch(() => {});
    ...
    await pool.query('DELETE FROM pickle_corporate_accounts WHERE id = $1', [req.params.id]);
...
app.get('/api/corporate-accounts/:id/appointments', async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(
      'SELECT * FROM pickle_appointment WHERE corporate_account_id = $1 ORDER BY preferred_date DESC',
      [id]
    );
...
app.post('/api/patient/lookup', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, full_name, phone_number, email, service_type, preferred_date, preferred_time, notes, status, cancel_token, created_at FROM pickle_appointment
...
app.post('/api/patient/cancel', async (req, res) => {
  try {
    ...
    const appointment = await pool.query(
      'SELECT * FROM pickle_appointment WHERE cancel_token = $1',
      [cancelToken]
    );
    ...
    await pool.query(
      `UPDATE pickle_appointment SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP, cancellation_reason = $1
...
app.put('/api/appointments/:id/reschedule', async (req, res) => {
  try {
    ...
    const oldAppointment = await pool.query(
      'SELECT * FROM pickle_appointment WHERE id = $1',
      [id]
    );
    ...
    const result = await pool.query(
      `UPDATE pickle_appointment SET preferred_date = $1, preferred_time = $2, updated_at = CURRENT_TIMESTAMP
       WHERE id = $3
       RETURNING *`,
      [preferredDate, preferredTime, id]
    );
  ...
  try {
    ...
    for (const appointment of result.rows) {
      try {
        ...
        // Mark as reminded
        await pool.query(
          'UPDATE pickle_appointment SET reminder_sent = true WHERE id = $1',
          [appointment.id]
        );
      } catch (err) {
...
app.get('/api/reports/earnings', async (req, res) => {
  try {
    ...
    // 1. Get Cash/Direct payments FROM pickle_appointment that were completed on this date
    // (Assuming payment_method = 'cash' and status/transport_status = 'completed')
    const cashRes = await pool.query(`
      SELECT SUM(total_amount) as total, COUNT(*) as count FROM pickle_appointment 
...
app.get('/api/reports/stats', async (req, res) => {
  try {
    ...
    const statusStats = await pool.query(
      `SELECT status, COUNT(*) as count FROM pickle_appointment ${dateFilter} GROUP BY status`,
      params
    );

    // Appointments by service type
    const serviceStats = await pool.query(
      `SELECT service_type, COUNT(*) as count FROM pickle_appointment ${dateFilter} GROUP BY service_type ORDER BY count DESC`,
      params
    );

    // Daily appointment count (last 30 days)
    const dailyStats = await pool.query(
      `SELECT preferred_date as date, COUNT(*) as count FROM pickle_appointment
    ...
    const hourlyStats = await pool.query(
      `SELECT preferred_time as time, COUNT(*) as count FROM pickle_appointment ${dateFilter}
    ...
    const totals = await pool.query(
      `SELECT COUNT(*) as total,
        ...
        COUNT(CASE WHEN status = 'confirmed' THEN 1 END) as confirmed FROM pickle_appointment ${dateFilter}`,
      params
    );
...
app.get('/api/export/appointments', async (req, res) => {
  try {
    ...
    let sql = 'SELECT * FROM pickle_appointment WHERE 1=1';
    const params = [];
    let paramIndex = 1;
...
app.get('/api/reports/trips', async (req, res) => {
  try {
    const { startDate, endDate, status } = req.query;
    let sql = `
      SELECT a.*, r.name as rider_name, r.plate_number, r.vehicle_type FROM pickle_appointment a
      LEFT JOIN riders r ON a.rider_id = r.id
  ...
  try {
    ...
    const appointments = await pool.query(
      `SELECT a.id, a.full_name, a.phone_number, a.service_type,
              ...
              a.rider_id, r.name as rider_name, r.vehicle_type, r.plate_number FROM pickle_appointment a
       LEFT JOIN riders r ON a.rider_id = r.id
...
app.get('/api/corporate-accounts', async (req, res) => {
  try {
    const { rows } = await pool.query("SELECT * FROM pickle_corporate_accounts ORDER BY created_at DESC");
    res.json({ success: true, accounts: rows });
  } catch (error) {
...
app.get('/api/corporate-accounts/validate/:accountNumber', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT id, company_name, status, credit_limit, balance FROM pickle_corporate_accounts WHERE UPPER(account_number) = UPPER($1)', [(accountNumber || '').trim()]);
    if (rows.length > 0) {
...
app.post('/api/corporate-accounts', async (req, res) => {
  try {
    ...
    const existing = await pool.query('SELECT id FROM pickle_corporate_accounts WHERE account_number = $1', [account_number]);
    if (existing.rows.length > 0) {
...
app.put('/api/corporate-accounts/:id', async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE pickle_corporate_accounts SET account_number = $1, company_name = $2, contact_person = $3, contact_email = $4, contact_phone = $5, credit_limit = $6, status = $7
       WHERE id = $8`,
...
app.delete('/api/corporate-accounts/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await pool.query('DELETE FROM pickle_corporate_accounts WHERE id = $1', [id]);
    res.json({ success: true });
  } catch (error) {
...
app.post('/api/booking-services', async (req, res) => {
  try {
    const { name, duration, price, icon, category, base_fare, per_km_rate, is_active, variable_prices } = req.body || {};
    
    const query = 'INSERT INTO booking_services (name, duration, price, icon, category, base_fare, per_km_rate, is_active, variable_prices) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *';
    const values = [
      ...
      variable_prices ? JSON.stringify(variable_prices) : '[]'
    ];
...
app.put('/api/booking-services/:id', async (req, res) => {
  try {
    ...
    const query = `
      ...
      SET name = $1, duration = $2, price = $3, icon = $4, category = $5, base_fare = $6, per_km_rate = $7, is_active = $8, variable_prices = $9, updated_at = CURRENT_TIMESTAMP
      WHERE id = $10 RETURNING *`;
    const values = [name, duration, price, icon, category, base_fare, per_km_rate, is_active !== undefined ? is_active : true, variable_prices ? JSON.stringify(variable_prices) : '[]', id];
...
app.get('/api/rider/requests', async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT a.*, s.base_fare, s.per_km_rate FROM pickle_appointment a
      JOIN booking_services s ON a.service_type = s.name
      WHERE s.category ILIKE 'TRANSPORT' 
...
app.post('/api/rider/accept', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "UPDATE pickle_appointment SET rider_id = $1, transport_status = 'accepted', status = 'confirmed' WHERE id = $2 AND (rider_id IS NULL OR transport_status = 'unassigned') RETURNING *",
...
app.post('/api/rider/update-status', async (req, res) => {
  try {
    ...
    if (status === 'completed') {
      ...
      const { rows: trip } = await pool.query('SELECT rider_id, total_amount FROM pickle_appointment WHERE id = $1', [appointmentId]);
    ...
    await pool.query(
      'UPDATE pickle_appointment SET transport_status = $1, status = $2, updated_at = NOW() WHERE id = $3',
...
app.post('/api/rider/sos', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE pickle_appointment SET transport_status = $1 WHERE id = $2', ['sos', tripId]);
...
app.get('/api/rider/history/:riderId', async (req, res) => {
  try {
    ...
    const { rows: history } = await pool.query(
      "SELECT * FROM pickle_appointment WHERE rider_id = $1 AND transport_status = 'completed' ORDER BY updated_at DESC",
      [riderId]
    );
...
app.get('/api/admin/riders/:id/trips', async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(
      "SELECT * FROM pickle_appointment WHERE rider_id = $1 ORDER BY created_at DESC",
      [id]
    );
    res.json({ success: true, trips: rows });
  } catch (error) {
...
app.get('/api/admin/clients', async (req, res) => {
  try {
    const { rows } = await pool.query(`
      ...
      FROM clients c
      LEFT FROM pickle_appointment a ON a.phone_number = c.phone_number
      GROUP BY c.id
...
app.get('/api/admin/clients/:phone/history', async (req, res) => {
  try {
    ...
    if (phoneParam === 'Unknown') {
      query = "SELECT * FROM pickle_appointment WHERE TRIM(phone_number) = '' OR phone_number IS NULL ORDER BY created_at DESC";
      params = [];
    } else {
      query = "SELECT * FROM pickle_appointment WHERE TRIM(phone_number) = $1 ORDER BY created_at DESC";
      params = [phoneParam];
    }
...
app.get('/api/admin/trips', async (req, res) => {
  try {
    ...
    let query = `
      SELECT a.*, r.name as rider_name, r.plate_number, r.vehicle_type, r.current_lat as rider_lat, r.current_lng as rider_lng FROM pickle_appointment a
      LEFT JOIN riders r ON a.rider_id = r.id
      JOIN booking_services s ON a.service_type = s.name
      WHERE s.category ILIKE 'TRANSPORT'
    `;
  ...
  try {
    const statsQuery = `
      SELECT COUNT(*) as total,
        ...
        COUNT(*) FILTER (WHERE a.transport_status = 'sos') as sos FROM pickle_appointment a
      JOIN booking_services s ON a.service_type = s.name
      WHERE s.category ILIKE 'TRANSPORT'
    `;
...
app.get('/api/admin/incidents', async (req, res) => {
  try {
    const { rows } = await pool.query(`
      ...
      FROM trip_incidents i FROM pickle_appointment a ON i.trip_id = a.id
...
app.post('/api/admin/trips/:id/timeline', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE pickle_appointment SET transport_status = $1 WHERE id = $2', [status, id]);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ success: false });
  }
...
app.post('/api/patient/lookup', async (req, res) => {
  try {
    const { email, referenceId } = req.body;
    const { rows } = await pool.query(
      'SELECT * FROM pickle_appointment WHERE email = $1 AND id = $2',
      [email, referenceId]
    );
  ...
  try {
    ...
    const { rows } = await pool.query(`
      SELECT a.*, r.name as rider_name, r.phone as rider_phone, 
             ...
             r.current_lat, r.current_lng, r.last_updated as rider_last_ping FROM pickle_appointment a
      LEFT JOIN riders r ON a.rider_id = r.id
...
const initClinicSettings = async () => {
  ...
  // Migrate existing walk-in clients FROM pickle_appointment to clients table
  await pool.query(`
...
app.get('/api/corporate-accounts', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM pickle_corporate_accounts ORDER BY id DESC');
    res.json({ success: true, accounts: rows });
  } catch (error) {
...
app.get('/api/corporate-accounts/dashboard-stats', async (req, res) => {
  try {
    ...
    const recRes = await pool.query('SELECT COALESCE(SUM(balance), 0) as total FROM pickle_corporate_accounts');
...
app.post('/api/corporate-accounts', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO pickle_corporate_accounts (account_number, company_name, contact_person, contact_email, contact_phone, credit_limit, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
...
app.put('/api/corporate-accounts/:id', async (req, res) => {
  try {
    ...
    const existing = await pool.query('SELECT id FROM pickle_corporate_accounts WHERE account_number = $1 AND id != $2', [account_number, id]);
    if (existing.rows.length > 0) {
      return res.status(400).json({ success: false, message: 'Account number already exists for another company' });
    }

    const result = await pool.query(
      `UPDATE pickle_corporate_accounts SET 
...
app.delete('/api/corporate-accounts/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM pickle_corporate_accounts WHERE id = $1 RETURNING id', [id]);
    if (result.rows.length === 0) return res.status(404).json({ success: false, message: 'Account not found' });
...
app.get('/api/corporate-accounts/:accountId/invoices/:invoiceId', async (req, res) => {
  try {
    ...
    const accResult = await pool.query(
      'SELECT company_name, account_number, contact_person, contact_email, contact_phone FROM pickle_corporate_accounts WHERE id = $1',
      [accountId]
    );
    ...
    const lineItems = await pool.query(
      `SELECT id, full_name as passenger,
              ...
              updated_at::text as completed_at FROM pickle_appointment
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT id, full_name as passenger, preferred_date::text as date, 
              COALESCE(pickup_location || ' → ' || destination_location, service_type) as route,
              total_amount as fare, COALESCE(billing_status, 'unbilled') as status FROM pickle_appointment 
...
app.post('/api/corporate-accounts/:id/invoices/generate', async (req, res) => {
  ...
  try {
    ...
    const trips = await client.query(
      `SELECT id, total_amount FROM pickle_appointment 
    ...
    await client.query(
      ` FROM pickle_appointment SET billing_status = 'billed', invoice_id = $1 WHERE id = ANY($2::int[])`,
    ...
    await client.query(
      ` FROM pickle_corporate_accounts SET balance = balance + $1 WHERE id = $2`,
      [totalAmount, id]
    );
...
app.post('/api/corporate-accounts/:id/ledger/adjust', async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `UPDATE pickle_corporate_accounts SET balance = balance + $1 WHERE id = $2`,
...
const autoQueueAppointments = async () => {
  try {
    ...
    const { rows: appointments } = await pool.query(
      `SELECT * FROM pickle_appointment WHERE preferred_date = $1 AND status IN ('pending', 'confirmed')`,
      [todayStr]
    );
    ...
    for (const appt of appointments) {
      ...
      if (currentTotalMinutes >= apptTotalMinutes - 10) {
        ...
        await pool.query(
          `UPDATE pickle_appointment SET status = 'queued', updated_at = NOW() WHERE id = $1`,
          [appt.id]
        );
...
app.post('/api/user/register', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [fullName, email || null, phoneNumber, hashedPassword, userRole]
    );
...
app.post('/api/user/login', async (req, res) => {
  try {
    ...
    const user = { 
      ...
      paymaya_number: rows[0].paymaya_number,
      bank_account: rows[0].bank_account,
      bank_account_name: rows[0].bank_account_name
    };
...
app.get('/api/owner/bookings/:email', async (req, res) => {
  try {
    const { email } = req.params;
    const { rows } = await pool.query(
      `SELECT a.*, a.preferred_date as appointment_date, a.preferred_time as appointment_time 
       FROM pickle_appointment a
...
// Unblock a slot
app.post('/api/owner/slots/unblock', async (req, res) => {
  try {
    const { courtName, date, time } = req.body;
    
    await pool.query(
      `DELETE FROM pickle_appointment 
       WHERE service_type = $1 AND preferred_date = $2 AND preferred_time = $3 AND status = 'blocked'`,
      [courtName, date, time]
    );
...
app.post('/api/owner/appointments/cancel', async (req, res) => {
  try {
    const { courtName, date, time } = req.body;
    ...
    await pool.query(
      `UPDATE pickle_appointment 
       ...
       WHERE service_type = $1 AND preferred_date = $2 AND preferred_time = $3 AND status != 'cancelled' AND status != 'blocked'`,
      [courtName, date, time]
    );
...
pool.query('ALTER TABLE pickle_courts ADD COLUMN IF NOT EXISTS day_discount_rate NUMERIC').catch(() => {});
...
// Get owner email by court name (must be before /:email to avoid route conflict)
app.get('/api/courts/owner-by-name', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0 || !result.rows[0].owner_email) {
      return res.json({ success: true, owner_email: 'rodge1109@yahoo.com' }); // fallback to default owner
    }
    res.json({ success: true, owner_email: result.rows[0].owner_email });
  } catch (error) {
    console.error('Error fetching court owner:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch court owner' });
  }
});

app.get('/api/courts/:email', async (req, res) => {
  try {
    const { email } = req.params;
    const result = await pool.query(
...
app.delete('/api/courts/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM pickle_courts WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Court not found' });
    }
...
app.post('/api/messages', async (req, res) => {
  try {
    const { senderEmail, receiverEmail, courtName, message } = req.body;
    const result = await pool.query(
      `INSERT INTO pickle_messages (sender_email, receiver_email, court_name, message) 
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [senderEmail, receiverEmail, courtName, message]
    );
  ...
  try {
    ...
    const query = `
      WITH RankedMessages AS (
        SELECT 
          m.*,
          CASE WHEN sender_email = $1 THEN receiver_email ELSE sender_email END as partner_email,
          ROW_NUMBER() OVER(
            PARTITION BY CASE WHEN sender_email = $1 THEN receiver_email ELSE sender_email END 
            ORDER BY created_at DESC
          ) as rn
        FROM pickle_messages m
        WHERE sender_email = $1 OR receiver_email = $1
      ),
      UnreadCounts AS (
        SELECT sender_email as partner_email, COUNT(*) as unread_count
        FROM pickle_messages
      ...
      LEFT JOIN users u ON rm.partner_email = u.email
      LEFT JOIN UnreadCounts uc ON rm.partner_email = uc.partner_email
      WHERE rm.rn = 1
...
app.get('/api/messages/chat/:email1/:email2', async (req, res) => {
  try {
    const { email1, email2 } = req.params;
    
    // Mark as read where email1 is the receiver and email2 is the sender
    await pool.query(
      'UPDATE pickle_messages SET is_read = true WHERE receiver_email = $1 AND sender_email = $2',
      [email1, email2]
    );
    
    const result = await pool.query(
      `SELECT * FROM pickle_messages 
...
app.get('/api/messages/unread/:email', async (req, res) => {
  try {
    const { email } = req.params;
    const result = await pool.query(
      'SELECT COUNT(*) as count FROM pickle_messages WHERE receiver_email = $1 AND is_read = false',
      [email]
    );
...
app.get('/api/notifications/:email', async (req, res) => {
  try {
    const { email } = req.params;
    const { rows } = await pool.query(
      'SELECT * FROM pickle_notifications WHERE user_email = $1 ORDER BY created_at DESC',
      [email]
    );
...
app.post('/api/notifications', async (req, res) => {
  try {
    const { user_email, sender_email, title, message } = req.body;
    await pool.query(
      'INSERT INTO pickle_notifications (user_email, sender_email, title, message) VALUES ($1, $2, $3, $4)',
      [user_email, sender_email, title, message]
    );
...
app.put('/api/notifications/:id/read', async (req, res) => {
  try {
    const { id } = req.params;
    await pool.query('UPDATE pickle_notifications SET is_read = TRUE WHERE id = $1', [id]);
    res.json({ success: true });
  } catch (error) {
...
// POST /api/open-plays/join
app.post('/api/open-plays/join', upload.single('proofOfPayment'), async (req, res) => {
  try {
    ...
    if (appointmentIds.length === 0 || !userEmail) {
      return res.status(400).json({ success: false, message: 'Missing required fields' });
    }

    // First check capacity for ALL requested slots
    ...
    // If all checks pass, insert participants for all slots
    for (const id of appointmentIds) {
      try {
        await pool.query(
    ...
    res.json({ success: true, message: 'Successfully joined open play' });
  } catch (err) {
    console.error('Join open play error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
...
app.get('/api/owner-earnings/:email', async (req, res) => {
  try {
    ...
    const transactions = rows.map(row => {
      ...
      const serviceFee = amount * 0.065;
      const netEarnings = amount - serviceFee;
    ...
    const totalServiceFee = grossEarnings * 0.065;
    const totalNetEarnings = grossEarnings - totalServiceFee;
