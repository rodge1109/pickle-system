...
function AppointmentForm() {
  ...
  const calculateFees = () => {
    ...
    if (cat === 'TRANSPORT') {
      ...
      let activePriceStr = (selectedService.price || '0').toString();
      
      // Handle variable pricing
      if (selectedService.variable_prices && selectedService.variable_prices.length > 0 && formData.preferredTime && formData.preferredTime.length > 0) {
        // Find the first selected time's hour (e.g. from "1:00 PM - 2:00 PM")
        const selectedTimeBlock = formData.preferredTime[0];
        const selectedHour = selectedTimeBlock.split('-')[0].trim();
        
        const matchedVp = selectedService.variable_prices.find(vp => vp.hour === selectedHour || selectedTimeBlock.includes(vp.hour));
        if (matchedVp) {
          activePriceStr = (matchedVp.price || '0').toString();
        }
      }

      flatPrice = parseFloat(activePriceStr.replace(/[^\d.]/g, '')) || 0;
      subtotal = flatPrice;
    }
  ...
  useEffect(() => {
    const fetchAvailableSlots = async () => {
      if (!formData.preferredDate || !selectedService?.name) {
        setAvailableSlots([]);
        return;
      }
      ...
      try {
        const url = `/api/available-slots?date=${formData.preferredDate}&serviceType=${encodeURIComponent(selectedService.name)}`;
    ...
    fetchAvailableSlots();
  }, [formData.preferredDate, selectedService?.name, confirmedAppointment, isSubmitting]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (allSuccess) {
        // Just take the first one for confirmation display
        const firstSuccess = results.find(res => res.success);
        setConfirmedAppointment(firstSuccess.appointment);
        setSubmitStatus({ type: 'success', message: 'Appointments booked successfully!' });
      } else if (anySuccess) {
        const firstSuccess = results.find(res => res.success);
        setConfirmedAppointment(firstSuccess.appointment);
        setSubmitStatus({ type: 'success', message: 'Some appointments were booked, but others failed.' });
      } else {
        setSubmitStatus({ type: 'error', message: results[0]?.message || 'Failed to book appointment' });
      }
    } catch (error) {
  ...
  return (
    <div ref={formTopRef} className="overflow-hidden flex flex-col min-h-[700px] scroll-mt-24">
      ...
      {/* Main Content Area */}
      <div className="flex-1 px-0 py-6 sm:p-6 md:p-12 overflow-y-auto">
        {step === 'service' && (
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-6">
              {isLoadingServices ? (
                ...
                >
                  ...
                  <div className="h-[150px] sm:h-72 w-full bg-gray-50 flex items-center justify-center relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-white/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20"></div>
                    
                    {(() => {
                      const isImage = service.icon && (service.icon.startsWith('http') || service.icon.startsWith('/uploads'));
                      return (
                        <div className={`flex items-center justify-center text-5xl transform group-hover:scale-110 transition-all duration-700 ease-out z-10 ${isImage ? 'w-full h-full' : 'w-24 h-24 sm:w-52 sm:h-52 drop-shadow-xl group-hover:-translate-y-2'}`}>
                          <ServiceIconRender 
                            iconName={service.icon} 
                            className={isImage ? "w-full h-full object-cover" : "w-20 h-20 sm:w-36 sm:h-36 object-contain"} 
                          />
                        </div>
                      );
                    })()}
                    ...
                    {/* Category Tag */}
                    <div className="absolute top-2 left-2 sm:top-4 sm:left-4 bg-white/90 backdrop-blur-sm px-2 py-0.5 sm:px-3 sm:py-1 text-[7px] sm:text-[8px] font-black uppercase tracking-[2px] text-[#525252] border border-gray-100 shadow-sm">
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-1.5 sm:gap-2">
                      <h4 className="font-black text-[#161616] uppercase tracking-tighter text-[12px] sm:text-[15px] leading-tight group-hover:text-[#24a148] transition-colors">
                        {service.name}
                      </h4>
                      <div className="text-[#24a148] font-extrabold text-[11px] sm:text-sm whitespace-nowrap bg-green-50 px-2 py-0.5 sm:py-1 border border-green-100 w-fit">
                        {service.price}
                      </div>
                    </div>
        ...
        {step === 'datetime' && (
          <div className="animate-fadeIn">
                ...
                {!formData.preferredDate ? (
                  ...
                  <>
                      ...
                      {allTimeSlots.map(time => {
                        ...
                        return (
                          <button
                            ...
                            className={`py-2 flex flex-col items-center justify-center gap-1 text-[11px] font-bold border transition-all uppercase tracking-tight h-14 ${
                              isBooked 
                                ? 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed opacity-60' 
                                : formData.preferredTime && formData.preferredTime.includes(time) 
                                  ? 'bg-[#24a148] text-white border-[#24a148] shadow-md' 
                                  : 'bg-white text-[#161616] border-[#e0e0e0] hover:border-[#8d8d8d]'
                            }`}
                          >
                            <span>{time}</span>
                            {isBooked && (
                              <span className="text-[7px] bg-gray-200 text-gray-500 border border-gray-300 px-1.5 py-0.5 rounded-sm uppercase tracking-widest font-black leading-none">
                                Booked
                              </span>
                            )}
                          </button>
                    ...
                    {formData.preferredTime && formData.preferredTime.length > 0 && (
                      <div className="mt-2 text-[#24a148] text-xs font-bold text-center">
            ...
            <div className="mt-12 flex justify-between items-center">
              ...
              <button
                disabled={!formData.preferredDate || !formData.preferredTime || formData.preferredTime.length === 0}
